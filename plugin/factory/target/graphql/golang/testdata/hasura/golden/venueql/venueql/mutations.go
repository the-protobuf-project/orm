...
func (h *mutationHandler) UpdateSById(ctx context.Context, id string, patch UpdateInput, req ...*UpdateSByIdRequest) (*VenueMutationResponse, error) {
	...
	args["_set"] = graphql.Var(graphql.SetColumns(patch), "VenueSetInput")
	if !graphql.IsOmitted(r.GetPreCheck()) {
		args["preCheck"] = graphql.VarPtr(r.GetPreCheck(), "VenueBoolExp")
	}
	res := <-h.gql.MutateFields(ctx, "updateVenuesById", &out, args)
	return out, res.Error
}
