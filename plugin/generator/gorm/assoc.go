...
func assocPlan(db *schema.Database, s *schema.Schema, t *schema.Table) ([]belongsTo, []hasManyField) {
	...
	for _, col := range t.Columns {
		...
		if inThisSchema(col.FKModel) {
			bts = append(bts, belongsTo{
				Col:    col,
				Field:  uniqueGoName(naming.PascalGo(naming.StripIDSuffix(col.Name)), used),
	...
	for _, hm := range t.HasMany {
		...
		hms = append(hms, hasManyField{Ref: hm, Field: uniqueGoName(naming.PascalGo(hm.Field), used)})
	}
	return bts, hms
}
